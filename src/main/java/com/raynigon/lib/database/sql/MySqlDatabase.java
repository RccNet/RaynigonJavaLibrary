 ...
 * The MySqlDatabase is an implementation of the MySql Java Database Connector (JDBC)
...
public class MySqlDatabase extends SqlDatabase{
	...
	/** This Method start a connection to an MysqlServer 
	...
	public void connect(InetAddress taddress, String tdbname, String tusername, String tpassword, int tport) throws SQLException{
		try {
			...
			new Driver();
			String condata = "jdbc:mysql://"+address.getHostAddress()+":"+String.valueOf(port)+"/"+dbname;
			conn = DriverManager.getConnection(condata, username, password);
	...
	public void reconnect(){
		...
		try {
			...
			if(!reconnect_sheduled){
				...
				reconnect_timer.schedule(new TimerTask(){@Override public void run() {reconnect();}}, 1000);
			}
		}
	}
